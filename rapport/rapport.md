...
Pour corriger ces défauts visuels tout en conservant les bonnes performances, nous avons réécrit le background avec **PixiJS**, une bibliothèque 2D s’appuyant sur **WebGL**. PixiJS facilite la gestion d’une scène graphique (sprites, textures, boucle de rendu) et exploite l’accélération matérielle pour déléguer au GPU les opérations coûteuses, ce qui réduit la pression sur le CPU et stabilise le rendu. Pour donner un ordre d'idée PixiJS nous permet d'utilise le canva comme une vrai interface, et peux être utilisé pour faire de jeux vidéo ou render des objets 3D sans grande performences.

En réécrivant le code Typescript avec l’intégration de PixiJS, on s'assurer de l’optimisation mais une meilleur gestion des resize et de la qualité visuelle. Nous avons ajouté un **listener** sur la largeur de l’écran pour adapter dynamiquement la scène au device : le background se redimensionne proprement, conserve la netteté au zoom et s’ajuste correctement aux formats non‑16:9. Les motifs de fond sont désormais rendus comme des textures WebGL, ce qui évite la pixellisation lors du zoom et garantit une meilleure qualité sur écrans haute densité. De plus, l'animation est devenue super fluide, le cap à 4 fps levé pour un taux de rafraichissement fluide mais correct géré par la lib.
...
L'adaptation du code a été plutot simple car beaucoup de la logique des canva a été retenu, avec quelques exceptions, mais nous avons facilement pour reprendre nos repere et adapter le code avec les types proposé par la lib. Aujourd'hui je sais que si je dois concevoir des interfaces pour des petits moteurs ou des animations, j'utiliserai PixiJS sans hésité, car bien plus poussé et facile que le canva classique html et un code maison. Une difficulté rencontré que je n'avais jamais su faire corrigé à la premiere version d'UnlockIt a d'ailleur été corrigée et consulté dans la partie <a href="#273--symétrie-du-background">2.7.3 : Symétrie du Background</a>
