...
const PUB_ID_PARAM = ApiParam({ name: 'id', type: Number, description: 'Numeric ID of the publisher.', example: 1 });
...
export const PublishersControllerDoc = {
  ...
  Update: () => applyDecorators(
    ApiAuth(),
    ApiOperation({ summary: 'Update a publisher', description: 'Admin only.' }),
    PUB_ID_PARAM,
  ...
  Remove: () => applyDecorators(
    ApiAuth(),
    ApiOperation({ summary: 'Delete a publisher', description: 'Admin only. Cascade-unlinks from all games.' }),
    PUB_ID_PARAM,
