...
const TAG_ID_PARAM = ApiParam({ name: 'id', type: Number, description: 'Numeric ID of the tag.', example: 1 });
...
export const TagsControllerDoc = {
  ...
  Remove: () => applyDecorators(
    ApiAuth(),
    ApiOperation({ summary: 'Delete a tag', description: 'Admin only. Cascade-unlinks from all games.' }),
    TAG_ID_PARAM,
